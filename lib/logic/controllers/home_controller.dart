...
class HomeController extends GetxController {
  bool wonCarsLoading = true;
  bool buyLoading = true;
  List<CarModel>  wonCars = [];
  List<CarModel> buyCars = [];
     loadWonCars( ) async {
     wonCarsLoading = true;
    update();
    try {
      await FCIGetDataXApi().getWonCars().then((value) async{
        if (value != null) {
          wonCars = value;
          update();
        }
      });
      wonCarsLoading = false;
      update();
    } catch (e) {
      update();
    }
  }
    loadOutBidCars( ) async {
    buyLoading = true;
    update();
    try {
      await FCIGetDataXApi().getOutBidCars().then((value) async{
        if (value != null) {
          buyCars = value;
          update();
        }
